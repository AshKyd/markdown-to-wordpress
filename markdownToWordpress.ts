...
function template({posts}){
    ...
    <channel>
        ...
        <item>
            ...
            <guid isPermaLink="false">https://photos.ash.ms/?p=${i}</guid>
            ...
            <content:encoded><![CDATA[${
                marked(
                    ...
                    post.content)
                .replace(/"\/wp-content/g, '"https://ash.ms/wp-content')
            }]]></content:encoded>
            <excerpt:encoded><![CDATA[${post.excerpt || post.description}]]></excerpt:encoded>
            <wp:post_id>${i}</wp:post_id>
            <wp:post_date><![CDATA[${post.date.toISOString().substr(0,19).replace('T', ' ')}]]></wp:post_date>
            <wp:post_date_gmt><![CDATA[${post.date.toISOString().substr(0,19).replace('T', ' ')}]]></wp:post_date_gmt>
            <wp:post_modified><![CDATA[${(post.updated).toISOString().substr(0,19).replace('T', ' ')}]]></wp:post_modified>
            <wp:post_modified_gmt><![CDATA[${(post.updated).toISOString().substr(0,19).replace('T', ' ')}]]></wp:post_modified_gmt>
            ...
            ${(post.tags || []).map(tag => `<category domain="post_tag" nicename="${tag}"><![CDATA[${tag}]]></category>
            `).join('\n')}
            <wp:postmeta>
...
export default function markdownToWordpress({fileArray}){
    const categories = {};
    ...
    const posts = fileArray.map(({path, slug, content}) => {
        ...
        return {
            ...
            tags: (typeof parsed.data.tags === 'string' ? [parsed.data.tags] : parsed.data.tags) || []
        }
    })
